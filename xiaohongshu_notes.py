...
def main():
    ...
    total_raw_rows = len(df_ana)
    st.sidebar.markdown(f"**原始总笔记数**: {total_raw_rows}")
    filter_mask = pd.Series([True] * total_raw_rows)
    ...
    if 'publish_date' in df_ana.columns:
        ...
        if not valid_dates.empty:
            ...
            if isinstance(date_range, (date, pd.Timestamp)):
                ...
                start_date, end_date = date_range
            # 修改点：允许日期为空（NaT）的行通过筛选
            date_mask = ((df_ana['publish_date'].dt.date >= start_date) & (df_ana['publish_date'].dt.date <= end_date)) | (df_ana['publish_date'].isna())
            filter_mask &= date_mask
            st.sidebar.info(f"📅 日期筛选后笔记数: {filter_mask.sum()} (包含无发布日期记录)")
        ...
        if col_cn in df_ana.columns and not df_ana[col_cn].isna().all():
            ...
            if min_val < max_val:
                selected = st.sidebar.slider(
                    ...
                    key=f"slider_{col_cn}"
                )
                mask = (df_ana[col_cn] >= selected[0]) & (df_ana[col_cn] <= selected[1])
                filter_mask &= mask

    df_filtered = df_ana[filter_mask].copy()
    raw_filtered = raw_df[filter_mask] if filter_mask.dtype == bool else raw_df.iloc[filter_mask]

    st.sidebar.success(f"✅ 最终筛选后笔记数: {len(df_filtered)} 条")
    ...
    st.info(f"当前筛选后共 **{len(raw_filtered)}** 条笔记（原始总记录数：{total_raw_rows}，已应用侧边栏所有筛选条件）")
    ...
    if show_all:
        ...
        st.caption(f"✅ 已显示全部 {len(preview_show)} 条记录")
    else:
        st.dataframe(preview_show.head(100), column_config=column_config, width='stretch')
        st.caption("ℹ️ 仅显示前100行，勾选上方复选框可查看全部数据")
