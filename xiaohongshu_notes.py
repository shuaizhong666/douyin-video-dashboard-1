...
# ======================== 数据源配置 ========================
# ⚠️ 请将下面的 URL 替换为你 GitHub 仓库中的小红书数据文件 Raw 地址
GITHUB_RAW_URL = "https://raw.githubusercontent.com/shuaizhong666/douyin-video-dashboard-1/main/小红书笔记数据.xlsx"

# ======================== 加载函数 ========================
@st.cache_data(ttl=180, show_spinner="正在从 GitHub 加载小红书数据...")
        ...
        try:
            ...
            try:
                ...
                if df.empty:
                    ...
                    st.success(f"✅ 数据加载成功，共 {len(df)} 条笔记记录")
                return df
...
def preprocess_for_analysis(df):
    """
    预处理：转换数值类型、解析日期、处理用户昵称空值，添加姓名/工号兜底（若无则补空）
    对于无效昵称（为空或纯空白），强制所有指标为0，笔记ID置NaN，不计入发布数量。
    """
    if df.empty:
        return df
    df_work = df.copy()

    # 1. 确保姓名、工号列存在（若原数据没有则补空白，保持兼容性）
    ...
    # 2. 数值列转换，缺失填充0
    num_cols_cn = ['点赞数', '评论数', '分享数', '收藏数', '粉丝数']
    ...
    # 4. 对于无有效昵称的行，强制所有指标为0，笔记ID置NaN，不计入发布数量
    ...
    # 5. 创建分组键：有有效昵称则用昵称，无效昵称则用「姓名+工号」组合（若姓名工号均空则用行索引）
...
def get_author_aggregation(df):
    """
    基于预处理后的 df 进行聚合统计，返回包含 姓名、工号、用户昵称、发布数量、总点赞数、总评论数、总收藏数、粉丝数的 DataFrame
    """
...
def main():
    ...
    st.markdown(f"**数据源**：GitHub 仓库（实时同步） | 支持姓名/工号（可选），无小红书号用户自动归零")
    ...
    st.caption("以下统计基于左侧全局筛选后的数据 | 发布数量 = 有效笔记ID计数 | 无小红书号用户显示为(无小红书号)且各项指标为0")
    author_df = get_author_aggregation(df_filtered)
    if not author_df.empty:
        # 展示列顺序
        base_cols = ['姓名', '工号', '用户昵称', '发布数量']
        ...
        with tab1:
            ...
            top_publish_display = top_publish[display_cols_order]
            st.dataframe(top_publish_display, width='stretch')
            fig_pub = px.bar(top_publish, x='用户昵称', y='发布数量',
                             title="发布数量 Top10",
        ...
        with tab2:
            if '总点赞数' in author_df.columns:
                ...
                fig_likes = px.bar(top_likes, x='用户昵称', y='总点赞数',
                                   title="总点赞数 Top10",
    ...
    st.caption("基于原始全量数据统计，不受左侧筛选影响。无小红书号用户（用户昵称为空）发布数量恒为0。")
    ...
    if 'publish_date' in df_monitor.columns and 'author_group_key' in df_monitor.columns:
        ...
        if not valid_pub.empty:
            ...
            # 获取所有作者基本信息
            info_cols = ['author_group_key', '姓名', '工号', '用户昵称']
            ...
            display_cols = ['姓名', '工号', '用户昵称', count_label, '发布状态', '备注']
            ...
            total_notes = author_status[count_label].sum() if not author_status.empty else 0
                ...
                with st.expander("📹 点击查看笔记详情（验证发布数量）"):
                    ...
                    if author_list:
                        ...
                        if '笔记ID' in author_notes.columns:
                            display_cols.append('笔记ID')
                        if '笔记标题' in author_notes.columns:
                            display_cols.append('笔记标题')
                        if '发布日期' in author_notes.columns:
                            display_cols.append('发布日期')
                        if link_col and link_col in author_notes.columns:
                            display_cols.append(link_col)
                        if not display_cols:
                            display_cols = author_notes.columns.tolist()
                        for prefix in ['姓名', '工号']:
                            if prefix in author_notes.columns and prefix not in display_cols:
                        ...
                        st.dataframe(author_notes[display_cols], column_config=column_config, width='stretch')
            else:
    ...
    base_cols = ['姓名', '工号', '用户昵称', '笔记标题', '点赞数', '评论数', '分享数', '收藏数', '粉丝数', '发布日期']
    if link_col:
    ...
    st.sidebar.info(
        "**姓名/工号**：从Excel中读取，若缺失则显示空白（可选）。\n\n"
        "**无小红书号用户**：当「用户昵称」为空时，该用户所有指标自动为0，且发布数量不计入任何统计。\n\n"
        "**发布数量定义**：有效小红书号的笔记ID计数。\n\n"
        "**发布监控**：支持单天或范围模式，默认显示昨日数据。\n\n"
        "**笔记链接**：若Excel中包含‘笔记链接’列，自动显示可点击链接。"
    )
